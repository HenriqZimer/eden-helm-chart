# Contributing to Eden Helm Chart
...
helm template eden chart/

# Install in a test cluster
helm install eden-test chart/ --dry-run --debug
```
...
- Update `appVersion` in `Chart.yaml` in your PR when pinning the chart to a specific Eden image
