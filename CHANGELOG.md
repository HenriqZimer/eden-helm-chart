...
### Fixed
- README install commands now use a unique repo alias (`eden-helm-chart`) instead of the bare chart name, and pin `--version` explicitly.
...
### Added
- Initial release of the Eden Helm chart
- Deployment, Service, optional Ingress and PVC for the linuxserver.io Eden KasmVNC webtop image
...
- `extraVolumes`/`extraVolumeMounts` for mounting an existing ROMs library
- `streaming.enabled`/`streaming.brokerPort` for RomM's Emulator Streaming (eden-romm-integration mod)
