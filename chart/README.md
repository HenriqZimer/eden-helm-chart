...
[![Version: 1.1.0](https://img.shields.io/badge/Version-1.1.0-informational?style=flat-square)](https://github.com/HenriqZimer/eden-helm-chart)
...
Nintendo Switch emulator, served as a full desktop over the browser via KasmVNC.
...
```bash
git clone https://github.com/HenriqZimer/eden-helm-chart.git
cd eden-helm-chart

helm package chart/
helm install eden ./eden-1.1.0.tgz
```
...
### Eden crashes when launching a game (SIGBUS)

If the dashboard/UI works but a game crashes right after boot (check the container logs for
`Unhandled SIGBUS`), Eden's JIT recompiler may need more `/dev/shm` than the small
...
starts when `/dev/shm` is too small); not independently confirmed on Eden, but worth trying
...
[linuxserver.io GPU Configuration docs](https://docs.linuxserver.io/images/docker-eden/#gpu-configuration)):
...
If Eden itself crashes with `Unhandled SIGBUS` in the container logs right after boot (i.e. the
...
RomM's streaming integration does not support save states for Eden (Switch has no equivalent to
PCSX2/Dolphin/xemu's slot-based save states) — the RomM player hides the save-state UI for this
platform. In-game saves (NAND) still sync normally via the broker.
...
provided by a third-party [Docker Mod](https://github.com/LoneAngelFayt/eden-romm-integration)
...
- `broker_host` is called **server-to-server**, by RomM's own backend. Point
  ...
  `http://eden.<namespace>.svc.cluster.local:8000` — cluster-internal DNS
...
| `image.repository` | `lscr.io/linuxserver/eden` | Container image. |
